...
//              window.location.href = `http://localhost:8080/helpDesk/id?id=${userID}`;
              var url = new URL("http://localhost:8080/helpDesk/id"),
                  params = {id:userID}
              Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
              fetch(url).then(resp => resp.json()).then(data => {JSONToHTMLTable(data, "tblEmployee");
...
//              window.location.href = `http://localhost:8080/helpDesk/id?username=${username}`;
    ...
    }).then(resp => resp.json()).then(data => {JSONToHTMLTable(data, "tblEmployee");
